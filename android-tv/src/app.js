...
class TVPlayer {
  ...
  async renderTextAsBook(rawText, media) {
    if (!rawText) {
      console.warn('No text to render for book');
  ...
  async tryUnmute() {
    ...
    try {
      ...
      // If we have user interaction permission, this will work
      await this.videoEl.play();
      this.hideMuteIndicator();
    } catch (e) {
      // Autoplay policy prevents unmuting, keep muted
  ...
  enableSoundOnInteraction() {
    const enableAudio = async () => {
      ...
      // Unlock TTS Engine by playing a silent utterance
  ...
  async sendHeartbeat() {
    try {
      await fetch(`${API_BASE}/api/devices/${this.deviceId}/heartbeat`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${this.deviceToken}`,
